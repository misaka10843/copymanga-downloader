...
def get_url():
    ...
    if response.status_code != 200:
        ...
        response = requests.get(
            ...
            headers=headers)
        if response.status_code != 200:
            print("您的网络似乎也[italic red]不支持jsdelivr[/italic red]，我们只能请求GitHub原站了")
            response = requests.get(
    ...
    for i in url_list:
        ...
        print(list_num, '->', i)
        list_num = list_num + 1
    get_url_list_num = Prompt.ask("[italic yellow]您希望通过第几个域名来获取相关信息?[/italic yellow][italic green](默认0)[/italic green]")
...
def manga_search(manga_name):
    ...
    if response.status_code == 200:
        ...
        print("[italic yellow]返回的状态码是：%d[/italic yellow]" % response.status_code)
        sys.exit(0)
...
def manga_chapter_list():
    ...
    if manga_chapter.status_code == 200:
        ...
        if int(how_download) == 1:
            ...
            start_chapter = end_chapter = Prompt.ask("[bold yellow]下载第几话？[/bold yellow]")
    else:
        # *报告远程服务器无法连接的状态码
        print("[italic yellow]服务器似乎[/italic yellow][italic red]无法连接[/italic red][italic yellow]了qwq[/italic yellow]\n")
        print("[italic yellow]返回的状态码是：%d[/italic yellow]" % manga_chapter.status_code)
        sys.exit(0)
...
def manga_collection(offset):
    ...
    if Authorization == 0:
        print(
            "[italic yellow]您并未填写Authorization，请按照https://t.hk.uy/bdFu 填写并添加到settings.json中的authorization字段[/italic yellow]")
        return
    manga_search_list = ""
...
def manga_collection_backup():
    ...
    if response.status_code == 200:
        ...
        for line in manga_list:
            f.write('%s\n' % line["comic"]["name"])
        f.close()
        print("[italic green]写入完成！[/italic green][italic yellow](SCV请用Excel打开，编码为GBK)[/italic yellow]")
        welcome()
    else:
        # *报告远程服务器无法连接的状态码
        print("[italic yellow]服务器似乎[/italic yellow][italic red]无法连接[/italic red][italic yellow]了qwq[/italic yellow]\n")
...
def welcome():
    is_search = Prompt.ask("您是想搜索还是查看您的收藏？[italic yellow](0:导出收藏,1:搜索,2:收藏  默认1)[/italic yellow]", choices=["0", "1", "2"], default="1")
    if is_search == "2":
        manga_collection(0)
    elif is_search == "0":
        manga_collection_backup()
    else:
        manga_name = Prompt.ask("请输入漫画名称")
